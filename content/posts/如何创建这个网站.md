...
summary: "创建本网站的具体步骤"
authors: [dnasdw]
date: 2021-12-06T00:34:22+08:00
draft: false
---
...
## 创建或使用守望者仓库watcher_test

<https://github.com/new>

<https://github.com/dnasdw/watcher_test>

## 创建部署仓库draft

<https://github.com/new>

<https://github.com/dnasdw/draft>

## 创建评论仓库comments_test

<https://github.com/new>

<https://github.com/dnasdw/comments_test>

## 创建源文件仓库pages_draft

<https://github.com/new>

<https://github.com/dnasdw/pages_draft>

克隆到本地并新建站点

```Shell
git clone --recursive https://github.com/dnasdw/pages_draft.git
hugo new site pages_draft --force
cd pages_draft
```

创建.gitignore

内容来自<https://github.com/github/gitignore/blob/main/community/Golang/Hugo.gitignore>
...
```TOML
baseURL = "https://dnasdw.github.io/draft"
...
<https://github.com/dnasdw/watcher_test/settings/secrets/actions>

如果没有则配置token

<https://github.com/dnasdw/watcher_test/settings/secrets/actions/new>
...
```Shell
git clone --recursive https://github.com/dnasdw/watcher_test.git
cd watcher_test
git submodule add https://github.com/dnasdw/pages_draft.git pages_draft
```

创建github actions配置.github/workflows/draft-gh-pages.yml
...
Payload URL: https://webhook_workflow_dispatch-worker.dnasdw.workers.dev/api/github/repos/dnasdw/watcher_test/actions/workflows/draft-gh-pages.yml/dispatches
```
...
<https://github.com/dnasdw/draft/settings/pages>

页面地址

<https://dnasdw.github.io/draft/>
...
```Text
Only select repositories: dnasdw/comments_test
```
  ...
  [languages.zh-cn]
    ...
    [languages.zh-cn.menu]
      ...
      [[languages.zh-cn.menu.main]]
        ...
        post = ""
        name = ""
        url = "https://github.com/dnasdw/draft"
        title = "GitHub"
        weight = 9
    [languages.zh-cn.params]
      ...
      [languages.zh-cn.params.home]
        ...
        [languages.zh-cn.params.home.profile]
          ...
          gravatarEmail = ""
          # 主页显示头像的 URL
          avatarURL = "/images/dnasdw256.png"
      ...
      [languages.zh-cn.params.sponsor]
        ...
        link = "https://github.com/sponsors/dnasdw"
        # custom = "<a href=\"https://www.buymeacoffee.com/dnasdw\" target=\"_blank\"><img src=\"https://cdn.buymeacoffee.com/buttons/v2/default-yellow.png\" alt=\"Buy Me A Coffee\" style=\"height: 40px !important;width: 145 !important;\" ></a>"
...
[params]
  ...
  gitRepo = "https://github.com/dnasdw/pages_draft"
  ...
  [params.page]
    ...
    # 配置编辑文章的链接
    linkToEdit = "https://github.com/dnasdw/pages_draft/edit/master/content/{path}"
    ...
    [params.page.comment]
      ...
      # Remark42 comment 评论系统设置 (https://remark42.com/)
...
如果config中baseURL包含子目录, 则menu配置中的url不能以斜杠开始, 比如"所有文章"的url不可以为`/posts/`, 但可以为`posts/`或`./posts/`.

### 头像文件

创建`assets/images/dnasdw256.png`, 并将config中的头像路径改为`/images/dnasdw256.png`.

### 网站图标

准备一个512x512的图标, 比如去<https://www.flaticon.com/>搜索一个合适的.

去<https://realfavicongenerator.net/>上传图标, 下载得到图标配置包, 解压到static目录.

如果config中baseURL包含子目录, 则需要将`themes/DoIt/layouts/partials/head/link.html`复制到`layouts/partials/head/link.html`, 并将favicon部分的链接前加上`/子目录名`, 比如`href="/favicon.ico"`改为`href="/draft/favicon.ico"`.
...
{{- if not .Site.Params.app.noFavicon -}}
    {{- with .Site.Params.app.svgFavicon -}}
        <link rel="icon" href="{{ . }}">
    {{- else -}}
        <link rel="shortcut icon" type="image/x-icon" href="/draft/favicon.ico" />
        <link rel="icon" type="image/png" sizes="32x32" href="/draft/favicon-32x32.png">
        <link rel="icon" type="image/png" sizes="16x16" href="/draft/favicon-16x16.png">
    {{- end -}}
    <link rel="apple-touch-icon" sizes="180x180" href="/draft/apple-touch-icon.png">
    {{- with .Site.Params.app.iconColor -}}
        <link rel="mask-icon" href="/draft/safari-pinned-tab.svg" color="{{ . }}">
    {{- end -}}
    {{- if eq .Site.Params.enablePWA true -}}
        <link rel="manifest" href="/draft/site.webmanifest">
    {{- end -}}
{{- end -}}
```
...
之后就可以在文章的Front Matter中使用`authors: [dnasdw]`.
...
修改`static/site.webmanifest`, 添加`start_url`, 并填写`name`/`short_name`/`start_url`.
...
```JSON
{
    ...
    "icons": [
        {
            "src": "/draft/android-chrome-192x192.png",
            "sizes": "192x192",
            "type": "image/png"
        },
        {
            "src": "/draft/android-chrome-512x512.png",
            "sizes": "512x512",
            "type": "image/png"
        }
    ],
...
如果config中baseURL包含子目录, 则需要将`themes/DoIt/assets/js/sw.js`复制到`assets/js/sw.js`, 并将相关链接前加上`/子目录名`, 比如`/css/style.min.css`改为`/darft/css/style.min.css`; 同样修改static/site.webmanifest中的相关路径, 其中start_url从`/`改为`/子目录名/`, 比如`"start_url": "/"`改为`"start_url": "/draft/"`; 还需要将`themes/DoIt/layouts/partials/footer.html`复制到`layouts/partials/footer.html`, 并修改注册参数, 比如`.register('/sw.min.js', { scope: '/' })`改为`.register('/draft/sw.min.js', { scope: '/draft/' })`.

配置参考

```JavaScript
const BASE_CACHE_FILES = [
  '/draft/css/style.min.css',
  '/draft/js/theme.min.js',
  '/draft/site.webmanifest'
]
...
const OFFLINE_PAGE = '/draft/offline'
const NOT_FOUND_PAGE = '/draft/404.html'
```
